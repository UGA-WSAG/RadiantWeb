            ...
            if(functionality != null){
            	...
            	for(String value : values){
                	...
                	buf.append("<div id=\"preExist-" + oper.getId() + "-modelReference" + count + "\" value=\"" + attrval + "\">"
                            ...
                            + "<span id=\"remove" + oper.getId() + "\" onclick=\"remove('wsdl', " + oper.getId() + ", 'operation', 'modelReference', '" + value.replace('\'', '"') + "', 'preExist-" + oper.getId() + "-modelReference" + count + "')\" class=\"ui-icon ui-icon-close hove\" style=\"float:left\"></span></div></div>");
                	count++;
            	}
            }
    	...
    	if(message.getModelReference() != null) {
            ...
            for(String value : values){
            	...
            	buf.append("<div id=\"preExist- " + message.getId() + " -modelReference" + count + "\""
                        ...
                        + "<span id=\"remove" + message.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + message.getId() + ", 'message', 'modelReference', '" + value.replace('\'', '"') + "', 'preExist- " + message.getId() + " -modelReference" + count + "')\" style=\"float:left\"></span></div></div>");
            	count++;
            }
        }
        if (message.getLiftingSchemaMapping() != null){
            ...
            for(String value : values){
            	buf.append("<div id=\"preExist-" + message.getId() + "-liftingSchemaMapping\""
            			...
            			+ "<span id=\"remove" + message.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + message.getId() + ", 'message', 'liftingSchemaMapping', '" + value + "', 'preExist-" + message.getId() + "-liftingSchemaMapping')\" style=\"float:left\"></span></div></div>");
            }
        }
        if(message.getLoweringSchemaMapping() != null){
            ...
            for(String value : values){
            	buf.append("<div id=\"preExist-" + message.getId() + "-loweringShemaMapping\""
            			...
            			+ "<span id=\"remove" + message.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + message.getId() + ", 'message', 'loweringSchemaMapping', '" + value + "', 'preExist-" + message.getId() + "-loweringShemaMapping')\" style=\"float:left\"></span></div></div>");
            }
        } 
    }
    	...
    	if(complex.getModelReference() != null) {
            ...
            for(String value : values){
            	...
            	buf.append("<div id=\"preExist-" + complex.getId() + "-modelReference" + count + "\""
                        ...
                        + "value=\"" + value + "\" title=\"" + value + "\" onclick=\"openOWLNode('" + owlid + "')\" >" + name + "</span>"
                        + "<span id=\"remove" + complex.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + complex.getId() + ", 'complex', 'modelReference', '" + value.replace('\'', '"') + "', 'preExist-" + complex.getId() + "-modelReference" + count + "')\" style=\"float:left\"></span></div></div>");
            	count++;
            }
        }
	}
    	...
    	if(simple.getModelReference() != null) {
            ...
            for(String value : values){
            	...
            	buf.append("<div id=\"preExist-" + simple.getId() + "-modelReference" + count + "\""
                        ...
                        + "value=\"" + value + "\" title=\"" + value + "\" onclick=\"openOWLNode('" + owlid + "')\" >" + name + "</span>"
                        + "<span id=\"remove" + simple.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + simple.getId() + ", 'simple', 'modelReference', '" + value.replace('\'', '"') + "', 'preExist-" + simple.getId() + "-modelReference" + count + "')\" style=\"float:left\"></span></div></div>");
            	count++;
            }
        }
        if (simple.getLiftingSchemaMapping() != null){
            ...
            for(String value : values){
            	buf.append("<div id=\"preExist-" + simple.getId() + "-liftingSchemaMapping\""
            			...
            			+ "<span id=\"remove" + simple.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + simple.getId() + ", 'simple', 'liftingSchemaMapping', '" + value + "', 'preExist-" + simple.getId() + "-liftingSchemaMapping')\" style=\"float:left\"></span></div></div>");
            }
        }
        if(simple.getLoweringSchemaMapping() != null){
            ...
            for(String value : values){
            	buf.append("<div id=\"preExist-" + simple.getId() + "-loweringSchemaMapping\""
            			...
            			+ "<span id=\"remove" + simple.getId() + "\" class=\"ui-icon ui-icon-close hove\" onclick=\"remove('wsdl', " + simple.getId() + ", 'simple', 'loweringSchemaMapping', '" + value + "', 'preExist-" + simple.getId() + "-loweringSchemaMapping')\" style=\"float:left\"></span></div></div>");
            }
        }
	}
